...
const cardsContainerInner = document.querySelector(".price-cards .cards__inner");
const cards = Array.from(document.querySelectorAll(".price-card"));
const overlay = document.querySelector(".price-cards .overlay");
...
const createOverlayCta = (overlayCard, ctaEl) => {
  ...
  overlayCta.classList.add("price-cta");
...
const observer = new ResizeObserver((entries) => {
  entries.forEach((entry) => {
    ...
    if (cardIndex >= 0) {
      overlay.querySelector('.cards__inner').children[cardIndex].style.width = `${width}px`;
      overlay.querySelector('.cards__inner').children[cardIndex].style.height = `${height}px`;
    }
  });
});
...
const initOverlayCard = (cardEl) => {
  ...
  overlayCard.classList.add("price-card");
  createOverlayCta(overlayCard, cardEl.lastElementChild);
  overlay.querySelector('.cards__inner').append(overlayCard);
